...
- **Identity key**: `telegram_id` (**uuid**, despite the name) is the primary key for users and the foreign key threaded through every domain table; it is carried in the JWT and read as `payload.get("telegram_id")` in routes.
