...
function renderSlides(tplId, colors) {
  ...
  if (tplId === 'neon') {
    ...
    const illus = (icon) => icon ? `<div style="position:relative;width:74px;flex-shrink:0"><div style="position:absolute;right:-16px;bottom:-16px;width:108px;height:108px;border-radius:50%;background:radial-gradient(circle,${acc}3d,transparent 70%)"></div><img src="/icons3d/${icon}.png" style="position:relative;width:74px;display:block;filter:url(#czic) drop-shadow(0 0 8px ${acc}aa)" alt=""></div>` : '';
    const o = [];
    ...
    CONTENT.slides.forEach((s, i) => o.push(`<div class="cz-slide" style="${bg}">${top(i + 1)}<div style="position:relative;z-index:2;margin-top:8px">${num(i + 1)}<div style="margin-top:2px">${h(s.t, 15)}</div>${line}</div><div style="flex:1;display:flex;align-items:flex-end;gap:8px;position:relative;z-index:2"><p style="flex:1;font-size:9px;color:#9fb0cf">${s.x}</p>${illus(s.icon)}</div></div>`));
