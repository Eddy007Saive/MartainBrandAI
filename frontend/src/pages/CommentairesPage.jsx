...
function Thread({ post }) {
  ...
  const doReply = async (commentId) => {
    ...
    } catch (e) { toast.error(e.response?.data?.detail || t('comments.sendFailed')); }
  ...
  const act = async (kind, c) => {
    ...
    } catch (e) { toast.error(e.response?.data?.detail || t('comments.actionFailed')); }
  };

  if (loading) return <div className="py-6 text-center"><Loader2 className="w-5 h-5 animate-spin text-[#5B6CFF] inline" /></div>;
  if (!comments?.length) return <p className="py-5 text-center text-slate-500 text-[12.5px] font-inter">{t('comments.noCommentsFetched')}</p>;
        ...
        <div key={c.id} className={`rounded-xl border border-white/[0.06] bg-[#0a1120] p-3.5 ${c.isHidden ? 'opacity-50' : ''}`}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[13px] font-semibold text-slate-200">{c.from?.name || t('comments.userFallback')}</div>
              <div className="text-[11px] text-slate-500">{since(c.createdTime, t)}{c.isHidden ? ` · ${t('comments.hidden')}` : ''}</div>
            </div>
          </div>
          <p className="text-[13.5px] text-slate-200 mt-2 leading-relaxed">{c.message}</p>
          <div className="flex flex-wrap gap-2 mt-2.5">
            {c.canReply !== false && <button onClick={() => { setReplyTo(replyTo === c.id ? null : c.id); setText(''); }} className="text-[12px] font-medium px-2.5 py-1.5 rounded-lg border border-white/10 text-slate-300 hover:bg-white/[0.06]">{t('comments.reply')}</button>}
            {c.canLike !== false && <button onClick={() => act(c.isLiked ? 'unlike' : 'like', c)} className={`text-[12px] font-medium px-2.5 py-1.5 rounded-lg border inline-flex items-center gap-1.5 ${c.isLiked ? 'text-red-400 border-red-400/30 bg-red-500/10' : 'border-white/10 text-slate-300 hover:bg-white/[0.06]'}`}><Heart className="w-3.5 h-3.5" />{c.isLiked ? t('comments.liked') : t('comments.like')}{c.likeCount ? ` ${c.likeCount}` : ''}</button>}
            {c.canHide !== false && <button onClick={() => act(c.isHidden ? 'unhide' : 'hide', c)} className="text-[12px] font-medium px-2.5 py-1.5 rounded-lg border border-white/10 text-slate-300 hover:bg-white/[0.06] inline-flex items-center gap-1.5"><EyeOff className="w-3.5 h-3.5" />{c.isHidden ? t('comments.show') : t('comments.hide')}</button>}
            {c.canDelete && <button onClick={() => act('delete', c)} className="text-[12px] font-medium px-2.5 py-1.5 rounded-lg border border-white/10 text-slate-300 hover:text-red-400 hover:bg-red-500/10 inline-flex items-center gap-1.5"><Trash2 className="w-3.5 h-3.5" />{t('comments.delete')}</button>}
          </div>
          {replyTo === c.id && (
            <div className="flex gap-2 mt-2.5">
              <input autoFocus value={text} onChange={(e) => setText(e.target.value)} placeholder={t('comments.replyPlaceholder')}
            ...
            return (
              <div key={post.id} className="rounded-2xl border border-white/[0.06] bg-[#0f172a] overflow-hidden">
                <button onClick={() => setOpen(isOpen ? null : post.id)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-white/[0.02]">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[13.5px] text-slate-200 truncate">{(post.content || t('comments.postFallback')).slice(0, 70)}</div>
                    <div className="text-[11.5px] text-slate-500 mt-0.5">{post.accountUsername} · {since(post.createdTime, t)}</div>
                  </div>
