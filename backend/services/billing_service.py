...
# La remise de retention : 30 % pendant trois mois, une seule fois par compte.
COUPON_RETENTION = "retention-30-3m"
REMISE_PCT = 30
REMISE_MOIS = 3


def remise_deja_accordee(telegram_id: str) -> bool:
    """A-t-il deja obtenu la remise de retention ?

    La trace vit dans le journal des departs, pas dans une colonne a part :
    c'est deja l'endroit ou l'on ecrit ce qui s'est passe a ce moment-la, et
    une seconde source finirait par diverger de la premiere.

    Sans cette verification, la remise se redemande indefiniment : il suffit
    d'entamer une resiliation tous les trois mois pour ne jamais payer le
    plein tarif.
    """
    try:
        r = (supabase.table("resiliations").select("id").eq("telegram_id", telegram_id)
             .eq("issue", "retenue").like("detail", "remise%").limit(1).execute())
        return bool(r.data)
    except Exception as e:
        # Table absente ou illisible : on REFUSE la remise plutot que de
        # l'accorder a l'aveugle. Une remise accordee deux fois se rattrape mal.
        logger.warning(f"remise_deja_accordee {telegram_id}: {e}")
        return True


def _coupon_retention():
    """Le coupon Stripe, cree au premier besoin puis reutilise."""
    try:
        return stripe.Coupon.retrieve(COUPON_RETENTION)
    except Exception:
        pass
    try:
        return stripe.Coupon.create(
            id=COUPON_RETENTION, percent_off=REMISE_PCT, duration="repeating",
            duration_in_months=REMISE_MOIS, name=f"Rétention −{REMISE_PCT}% ({REMISE_MOIS} mois)")
    except Exception as e:
        logger.error(f"creation du coupon de retention: {e}")
        return None
...
def accorder_remise(telegram_id: str, raison: str = "prix",
    ...
    obtenu quelque chose qu'il ne recevrait pas, et s'en apercevrait sur sa
    prochaine facture — au pire moment, celui ou il hesitait deja a partir.
    """
    if remise_deja_accordee(telegram_id):
        return {"ok": False, "error": "Cette remise a déjà été accordée sur ce compte.",
                "deja": True}

    ligne, sub = _abonnement_courant(telegram_id)
    if not ligne:
        return {"ok": False, "error": "Aucun abonnement en cours."}
    if not sub:
        return {"ok": False, "error": "Remise indisponible sur cet abonnement."}
    if sub.status == "trialing":
        return {"ok": False, "error": "L'essai est déjà gratuit — rien à remiser."}

    coupon = _coupon_retention()
    if not coupon:
        return {"ok": False, "error": "Remise indisponible pour le moment."}

    try:
        # `discounts` remplace `coupon` dans les versions recentes de l'API ;
        # on retombe sur l'ancien parametre pour les comptes plus anciens.
        try:
            stripe.Subscription.modify(sub.id, discounts=[{"coupon": coupon.id}],
                                       cancel_at_period_end=False)
        except TypeError:
            stripe.Subscription.modify(sub.id, coupon=coupon.id, cancel_at_period_end=False)
    except Exception as e:
        logger.error(f"remise de retention {telegram_id}: {e}")
